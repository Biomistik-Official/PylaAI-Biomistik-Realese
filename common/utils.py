...
def refresh_brawl_stars_api_token_if_enabled(config, file_path="cfg/brawl_stars_api.toml", force=False):
    ...
    key_name_prefix = str(config.get("key_name_prefix", "Pyla-Biomistik Auto")).strip() or "Pyla-Biomistik Auto"
    ...
    description = str(config.get("key_description", "Auto-generated by Pyla-Biomistik for the current public IP."))
    create_payload = {
        "name": key_name,
...
def resolve_instance_path(path_str):
...
def load_toml_as_dict(file_path):
    file_path = resolve_instance_path(file_path)
...
brawlers_info_file_path = resolve_instance_path("cfg/brawlers_info.json")
...
async def async_notify_user(message_type: str | None = None, screenshot: Image = None, details=None) -> bool:
    ...
    results = await asyncio.gather(
        send_discord_notification(message_type, screenshot=screenshot, details=details),
        send_telegram_notification(message_type, screenshot=screenshot, details=details),
