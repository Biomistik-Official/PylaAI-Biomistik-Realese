...
from common.utils import load_toml_as_dict, count_hsv_pixels, load_brawlers_info
...
class Movement:
    ...
    def angle_from_direction(dx: float, dy: float) -> float:
        ...
        Uses screen coordinates: 0deg = right, 90deg = down, 180deg = left, 270deg = up.
        """
    ...
    def start_semicircle_escape(self, angle, current_time):
        ...
        self._wslog(f"semicircle escape START: angle={angle:.1f}deg "
                    f"retreat={self.escape_state['retreat_angle']:.1f}deg "
...
class Play(Movement):
    ...
    def __init__(self, main_info_model, tile_detector_model, window_controller):
        ...
        self.is_showdown = bot_config["gamemode"] in ["showdown", "brawlball", "brawl_ball"]
        self.is_brawlball = bot_config["gamemode"] in ["brawlball", "brawl_ball", "brawll ball"]
    ...
    def run_playstyle(self, player_data, enemy_data, walls, brawler):
        ...
        env = {
            ...
            # angle -> WASD string  (0deg=right D, 90deg=down S, 180deg=left A, 270deg=up W)
            "angle_to_keys": lambda angle: [
                "D", "SD", "S", "SA", "A", "WA", "W", "WD"
            ][int((float(angle) % 360 + 22.5) / 45) % 8],
            # wall-stuck detector (uses Movement's own stateful wall tracking)
            ...
            "start_escape": lambda angle: self.start_semicircle_escape(
    ...
    def detect_fog_threat(self, frame, player_position):
        ...
        vlog(f"fog threat: {count}px within {r}px -> flee angle={flee:.1f}deg (fog at {toward_fog:.1f}deg)")
        return flee
    ...
    def showdown_roam(self, player_data, walls):
        ...
        if current_blocked or time_expired:
            ...
            if getattr(self, 'is_brawlball', False):
                # In Brawl Ball, idle roaming should heavily favor pushing UP towards the enemy goal.
                # 270 = Up. 240/300 = Up-Left/Up-Right.
                preferred_angles = [270, 240, 300, 210, 330, 180, 0, 90]
                for candidate in preferred_angles:
            ...
            if self.roam_center_bias > 0 and not getattr(self, 'is_brawlball', False):
            ...
            vlog(f"roam: new direction -> {self._roam_angle:.1f}deg")

        vlog(f"roam: holding -> angle={self._roam_angle:.1f}deg")
    ...
    def showdown_follow_teammate(self, player_data, teammate_data, walls):
        ...
        vlog(f"{action} teammate -> angle={best:.1f}deg (desired={angle:.1f}deg, dist={int(closest_distance)}px, "
    ...
    def get_showdown_movement(self, player_data, enemy_data, teammate_data, walls, brawler):
        ...
        # --- DYNAMIC CLASS-BASED CONFIGS (RANGE-BASED) ---
        if attack_range <= 350:
            # ASSASSIN / CLOSE-RANGE TANK
            self.lead_shots_enabled = False
            self.aimed_attacks_enabled = False
            self.combat_dodge_blend = 0.25
            safe_range = 0  # Always push, ignore safety distance
        elif attack_range >= 520:
            # SNIPER / LONG-RANGE
            self.lead_shots_enabled = True
            self.aimed_attacks_enabled = True
            self.combat_dodge_blend = 0.65
            safe_range = attack_range * 0.8  # Keep maximum distance
        else:
            # BALANCED
            self.lead_shots_enabled = str(load_toml_as_dict("cfg/bot_config.toml").get("lead_shots", "yes")).lower() in ("yes", "true", "1")
            self.aimed_attacks_enabled = str(load_toml_as_dict("cfg/bot_config.toml").get("aimed_attacks", "no")).lower() in ("yes", "true", "1")

        # --- LOW HP STRATEGIC RETREAT ---
        is_low_hp = False
        if hasattr(self, 'current_frame') and self.current_frame is not None:
            green_hp, red_hp = self._entity_team_color_scores(self.current_frame, player_data[:4])
            total_hp_pixels = green_hp + red_hp
            if total_hp_pixels > 15: # Ensure we actually see a health bar
                hp_ratio = green_hp / total_hp_pixels
                if hp_ratio < 0.35:
                    is_low_hp = True
                    vlog(f"LOW HP DETECTED! Ratio: {hp_ratio:.2f}. Forcing Retreat!")
        ...
        fog_flee_angle = None
        if not getattr(self, 'is_brawlball', False):
        ...
        if not self.is_there_enemy(enemy_data):
            ...
            if enemy_coords is None:
                ...
                # Check if the enemy is actually hittable (not behind a wall)
                ...
                if is_low_hp:
                    # OVERRIDE: Low HP Retreat
                    desired = self.angle_opposite(toward_angle)
                    vlog(f"low hp override -> retreat desired={desired:.1f}deg")
                elif enemy_distance > safe_range:
                    desired = toward_angle
                    vlog(f"enemy detected -> approach desired={desired:.1f}deg (dist={int(enemy_distance)}px, safe={safe_range}px)")
                    if self.approach_flank_blend > 0 and enemy_distance > safe_range * 1.2:
                        ...
                        vlog(f"approach flank blend -> desired={desired:.1f}deg")
                else:
                    desired = self.angle_opposite(toward_angle)
                    vlog(f"enemy too close -> retreat desired={desired:.1f}deg (dist={int(enemy_distance)}px, safe={safe_range}px)")
                    if self.multi_enemy_flee_weight > 0 and enemy_data and len(enemy_data) > 1:
                        ...
                        if math.hypot(mass_dx, mass_dy) > 10:
                            ...
                            vlog(f"multi-enemy flee blend -> desired={desired:.1f}deg")
                ...
                if (
                    ...
                    vlog(f"strafe blend -> desired={desired:.1f}deg")
                elif (
                        self.strafe_enabled
                        and fog_flee_angle is None
                    ...
                    vlog(f"retreat strafe blend -> desired={desired:.1f}deg")

                if self.strafe_enabled and fog_flee_angle is None and enemy_distance <= attack_range and enemy_hittable:
                    desired = self.apply_combat_dodge(desired, toward_angle, now_t, enemy_distance, safe_range)
                    vlog(f"combat dodge blend -> desired={desired:.1f}deg")
                ...
                if (self.trio_grouping_enabled and teammate_data and enemy_distance > attack_range):
                    ...
                    if closest_teammate is not None and teammate_distance > self.teammate_combat_regroup_distance:
                        ...
                        vlog(f"combat regroup bias -> desired={desired:.1f}deg (team dist={int(teammate_distance)}px)")

                angle = self.find_best_angle(player_pos, desired, walls)
                vlog(f"showdown: movement angle={angle:.1f}deg (desired={desired:.1f}deg)")
        ...
        if fog_flee_angle is not None:
            ...
            vlog(f"showdown: fog override -> angle={angle:.1f}deg")
        ...
        vlog(f"showdown movement -> angle={angle:.1f}deg")
    ...
    def loop(self, brawler, data, current_time):
        playstyle_movement = self.run_playstyle(data['player'][0], data['enemy'], data['wall'], brawler)
        if playstyle_movement is not None:
            movement = playstyle_movement
            # Convert string directions to float if needed later, but do_movement handles WASD string fine!
    ...
    def show_visual_debug(self, frame, data, brawler=None, respect_throttle=True):
        ...
        cv2.imshow("Pyla-Biomistik Visual Debug", cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        cv2.waitKey(1)
