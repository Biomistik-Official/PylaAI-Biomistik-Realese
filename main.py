...
from common.logger_setup import setup_logging_if_enabled

setup_logging_if_enabled()

import vision.window_controller
from remote.discord_control import DiscordControlServer
from remote.telegram_control import TelegramControlServer
from gui.hub import Hub
from gui.login import login
from gui.main import App
from gui.select_brawler import SelectBrawler
from core.lobby_automation import LobbyAutomation
from core.play import Play
from remote.runtime_control import RuntimeControlWindow
from core.stage_manager import StageManager
from vision.state_finder import get_state, get_starr_nova_got_it_button_center, is_starr_nova_info_screen
from common.time_management import TimeManagement
...
def pyla_main(data):
    ...
    class Main:
        ...
        def __init__(self):
            ...
            self.telegram_control = TelegramControlServer(self.control_window.state_path)
            
            def get_screenshot():
                if self.window_controller:
                    frame_data = self.window_controller.get_latest_frame()
                    if frame_data and frame_data[0] is not None:
                        # get_latest_frame returns (frame(RGB), time). cv2.imencode expects BGR.
                        import cv2
                        return cv2.cvtColor(frame_data[0], cv2.COLOR_RGB2BGR)
                return None
            ...
            def get_stats():
                try:
                    ...
                    return {
                        "win_streak": obs.win_streak,
                        "trophies": obs.current_trophies,
                        "wins": obs.current_wins
                    }
                except Exception:
                    return {}
                    
            def get_queue():
                if hasattr(self, 'Stage_manager') and self.Stage_manager:
                    return self.Stage_manager.brawlers_pick_data
                return []
            ...
            self.discord_control.get_queue_cb = get_queue
            self.discord_control.skip_queue_cb = skip_queue
            self.discord_control.start()
            
            self.telegram_control.get_screenshot_cb = get_screenshot
            self.telegram_control.get_stats_cb = get_stats
            self.telegram_control.get_queue_cb = get_queue
            self.telegram_control.skip_queue_cb = skip_queue
        ...
        def manage_time_tasks(self, frame):
            ...
            if self.Time_management.state_check():
                ...
                if state == "match" and getattr(self, "reward_chain_seen", False):
