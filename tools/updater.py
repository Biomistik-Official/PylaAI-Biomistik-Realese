...
def request_json(url: str) -> dict:
    request = urllib.request.Request(url, headers={
        ...
        "User-Agent": "Pyla-Biomistik-Updater",
    })
...
def download_file(url: str, destination: Path, label: str) -> Path:
    print(f"Downloading latest Pyla-Biomistik update ({label})...")
    request = urllib.request.Request(url, headers={"User-Agent": "Pyla-Biomistik-Updater"})
...
def find_project_root(extracted_dir: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError("Downloaded update does not look like a Pyla-Biomistik project.")
...
def main() -> int:
    ...
    if not (project_dir / "main.py").exists():
        print("updater.exe must be inside the Pyla-Biomistik project folder next to main.py.")
        wait_for_enter()
        return 1

    if "--smoke-test" in sys.argv:
        print("Smoke test passed. Updater can see the Pyla-Biomistik project folder.")
        return 0
