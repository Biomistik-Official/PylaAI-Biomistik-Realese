...
def create_run_file(project_dir, python_command):
    ...
    run_bat = project_dir / "Run Pyla-Biomistik.bat"
    run_bat.write_text(
        "@echo off\n"
        "cd /d %~dp0\n"
...
def main():
    ...
    if not setup_py.exists() or not main_py.exists():
        print("setup.exe must be placed in the Pyla-Biomistik project folder next to setup.py and main.py.")
        input("Press Enter to close...")
        return 1
    ...
    print("")
    print("Pyla-Biomistik setup completed.")
    print("Start your emulator, open Brawl Stars, then run Run Pyla-Biomistik.bat or python main.py.")
    input("Press Enter to close...")
    return 0
